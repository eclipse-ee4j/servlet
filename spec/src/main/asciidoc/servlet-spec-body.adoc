...
link:https://github.com/eclipse-ee4j/servlet-api/pull/372[PR 373]::
Provide defaults for deprecated methods, to return either `null` or an empty `Enumeration`.
`HttpServletRequest#isRequestedSessionIdFromUrl` calls `isRequestedSessionIdFromURL` by default.
